...
Encoder: TypeAlias = (  # FIXME: use picamera2 base class
    H264Encoder | JpegEncoder | MJPEGEncoder
)
...
class PiCamera(Camera):
    _device: Picamera2
    _encoder: Encoder  # FIXME: do we need a factory?
    ...
    def start_video(self, path: Path) -> None:
        ...
        encoder.output = PyavOutput(
            str(path) + ".mp4"  # FIXME: suffix is encoder-specific!
        )
        self._device.start_encoder(encoder)
